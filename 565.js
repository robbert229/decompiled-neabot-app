var module566 = require('./566'),
  module567 = require('./567'),
  s = {};

for (var o in module566) module566.hasOwnProperty(o) && (s[module566[o]] = o);
...
u.get.rgb = function (n) {
  ...
  if ((s = n.match(/^#([a-f0-9]{6})([a-f0-9]{2})?$/i))) {
    ...
    if (!(s = n.match(/^rgba?\(\s*([+-]?[\d\.]+)\%\s*,\s*([+-]?[\d\.]+)\%\s*,\s*([+-]?[\d\.]+)\%\s*(?:,\s*([+-]?[\d\.]+)\s*)?\)$/)))
      return (s = n.match(/(\D+)/)) ? ('transparent' === s[1] ? [0, 0, 0, 0] : (h = module566[s[1]]) ? ((h[3] = 1), h) : null) : null;
...
u.get.hsl = function (t) {
  if (!t) return null;
  var n = t.match(/^hsla?\(\s*([+-]?(?:\d{0,3}\.)?\d+)(?:deg)?\s*,?\s*([+-]?[\d\.]+)%\s*,?\s*([+-]?[\d\.]+)%\s*(?:[,|\/]\s*([+-]?[\d\.]+)\s*)?\)$/);
...
u.get.hwb = function (t) {
  if (!t) return null;
  var n = t.match(/^hwb\(\s*([+-]?\d{0,3}(?:\.\d+)?)(?:deg)?\s*,\s*([+-]?[\d\.]+)%\s*,\s*([+-]?[\d\.]+)%\s*(?:,\s*([+-]?[\d\.]+)\s*)?\)$/);
