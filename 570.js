...
for (var h in module571) module571.hasOwnProperty(h) && (t[module571[h]] = h);
...
u.rgb.keyword = function (h) {
  ...
  for (var b in module571)
    if (module571.hasOwnProperty(b)) {
      s = h;
      c = f;
      var f = module571[b],
...
u.ansi256.rgb = function (n) {
  if (module571 >= 232) {
    var t = 10 * (module571 - 232) + 8;
    return [t, t, t];
  }

  var h;
  module571 -= 16;
  return [(Math.floor(module571 / 36) / 5) * 255, (Math.floor((h = module571 % 36) / 6) / 5) * 255, ((h % 6) / 5) * 255];
};
